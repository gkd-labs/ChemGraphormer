...
# Mean-pooling ablation variants -> model module (each defines a differently-structured
# ChemGraphormerClassifier). Use --variant to pick the one matching your checkpoint.
VARIANT_MODEL_MAP = {
    "mean_pool_gate_zero": "model.chemgraphormer_ablate_use_mean_pooling_gate_zero",
    "mean_pool_gate_one":  "model.chemgraphormer_ablate_use_mean_pooling_gate_one",
    "mean_pool_gate_two":  "model.chemgraphormer_ablate_use_mean_pooling_gate_two",
}
...
def make_prediction_meanpl(dataset_path, checkpoint_path, model_params=None, device="cuda", batch_size=128, model_class=None):
    """
    ...
    :param model_class: The ChemGraphormerClassifier class matching the checkpoint's
                         ablation variant. If None, defaults to the mean_pool_gate_one
                         variant (backward-compatible with earlier direct callers).
    """
    if model_class is None:
        model_class = load_model_class("mean_pool_gate_one")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run ChemGraphormer ablation (mean pooling) inference and save predictions to disk.")
    ...
    parser.add_argument("--variant",         type=str,   required=True, choices=list(VARIANT_MODEL_MAP.keys()),
                         help="Which mean-pooling ablation variant's architecture to load (must match how the checkpoint was trained).")
...
if __name__ == "__main__":
    ...
    preds_df, mae = make_prediction_meanpl(
