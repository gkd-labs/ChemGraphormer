...
from model.chemgraphormer_ablate_static_edge_flow import ChemGraphormerClassifier


def parse_args():
    parser = argparse.ArgumentParser(description="Static Edge Flow ablation training for ChemGraphormer with warmup + cosine LR decay.")
...
def run_static_edge_flow_ablation(args):
    """
    Full model with static edge flow (no gating mechanism) ablation training loop for ChemGraphormer with linear warmup + cosine LR decay,
    ...
    no_improve_epochs = 0
    best_ckpt_path = os.path.join(args.ckpt_dir, "chemgraphormer_static_edge_flow_ablation_best_model.pt")
...
if __name__ == "__main__":
    args = parse_args()
    run_static_edge_flow_ablation(args)