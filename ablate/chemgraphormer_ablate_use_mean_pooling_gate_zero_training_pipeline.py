...
from model.chemgraphormer_ablate_use_mean_pooling_gate_zero import ChemGraphormerClassifier


def parse_args():
    parser = argparse.ArgumentParser(description="Use Mean Pooling ablation training for ChemGraphormer with warmup + cosine LR decay.")
...
def run_use_mean_pooling_gate_zero_ablation(args):
    """
    Mean pooling ablation training loop for ChemGraphormer with linear warmup + cosine LR decay,
    ...
    no_improve_epochs = 0
    best_ckpt_path = os.path.join(args.ckpt_dir, "chemgraphormer_mean_pooling_gate_zero_ablation_best_model.pt")
...
if __name__ == "__main__":
    args = parse_args()
    run_use_mean_pooling_ablation(args)