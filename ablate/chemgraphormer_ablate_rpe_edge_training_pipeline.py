...
from model.chemgraphormer_ablate_rpe_edge import ChemGraphormerClassifier


def parse_args():
    parser = argparse.ArgumentParser(description="RPE and edge ablation training for ChemGraphormer with warmup + cosine LR decay.")
...
def run_rpe_edge_ablation(args):
    """
    Full model with no RPE and edge ablation training loop for ChemGraphormer with linear warmup + cosine LR decay,
    ...
    no_improve_epochs = 0
    best_ckpt_path = os.path.join(args.ckpt_dir, "chemgraphormer_rpe_edge_ablation_best_model.pt")
