...
from model.chemgraphormer_ablate_edge import ChemGraphormerClassifier


def parse_args():
    parser = argparse.ArgumentParser(description="Edge ablation training for ChemGraphormer with warmup + cosine LR decay.")
...
def run_edge_ablation(args):
    """
    Full model with no edge ablation training loop for ChemGraphormer with linear warmup + cosine LR decay,
    ...
    no_improve_epochs = 0
    best_ckpt_path = os.path.join(args.ckpt_dir, "chemgraphormer_edge_ablation_best_model.pt")
