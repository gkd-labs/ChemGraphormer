...
from model.chemgraphormer_ablate_gate_init_two import ChemGraphormerClassifier


def parse_args():
    parser = argparse.ArgumentParser(description="Gate init two ablation training for ChemGraphormer with warmup + cosine LR decay.")
...
def run_gate_init_two_ablation(args):
    """
    Full gate init two ablation training loop for ChemGraphormer with linear warmup + cosine LR decay,
    ...
    no_improve_epochs = 0
    best_ckpt_path = os.path.join(args.ckpt_dir, "chemgraphormer_gate_two_ablation_best_model.pt")
...
if __name__ == "__main__":
    args = parse_args()
    run_gate_init_two_ablation(args)