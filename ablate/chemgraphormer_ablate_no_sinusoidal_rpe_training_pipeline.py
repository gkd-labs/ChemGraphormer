...
from model.chemgraphormer_ablate_no_sinusoidal_rpe import ChemGraphormerClassifier


def parse_args():
    parser = argparse.ArgumentParser(description="No Sinusoidal RPE ablation training for ChemGraphormer with warmup + cosine LR decay.")
...
def run_no_sinusoidal_rpe_ablation(args):
    """
    Full no sinusoidal rpe ablation training loop for ChemGraphormer with linear warmup + cosine LR decay,
    ...
    no_improve_epochs = 0
    best_ckpt_path = os.path.join(args.ckpt_dir, "chemgraphormer_no_sinusoidal_rpe_ablation_best_model.pt")
...
if __name__ == "__main__":
    args = parse_args()
    run_no_sinusoidal_rpe_ablation(args)