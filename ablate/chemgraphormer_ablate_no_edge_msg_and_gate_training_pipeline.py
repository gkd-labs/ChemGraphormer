...
from model.chemgraphormer_ablate_no_edge_msg_and_gate import ChemGraphormerClassifier


def parse_args():
    parser = argparse.ArgumentParser(description="No Edge Messages and Gate ablation training for ChemGraphormer with warmup + cosine LR decay.")
...
def run_no_edge_msg_and_gate_ablation(args):
    """
    Full model with no edge message and gate ablation training loop for ChemGraphormer with linear warmup + cosine LR decay,
    ...
    no_improve_epochs = 0
    best_ckpt_path = os.path.join(args.ckpt_dir, "chemgraphormer_no_edge_msg_ablation_best_model.pt")
