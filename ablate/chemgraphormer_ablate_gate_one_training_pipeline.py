...
from model.chemgraphormer_ablate_gate_init_one import ChemGraphormerClassifier


def parse_args():
    parser = argparse.ArgumentParser(description="Gate initialization at 1 ablation training for ChemGraphormer with warmup + cosine LR decay.")
...
def run_gate_init_one_ablation(args):
    """
    Gate initialization at 1 ablation training loop for ChemGraphormer with linear warmup + cosine LR decay,
    ...
    no_improve_epochs = 0
    best_ckpt_path = os.path.join(args.ckpt_dir, "chemgraphormer_gate_one_ablation_best_model.pt")
...
if __name__ == "__main__":
    args = parse_args()
    run_gate_init_one_ablation(args)