...
from model.chemgraphormer_ablate_gate_init_zero import ChemGraphormerClassifier


def parse_args():
    parser = argparse.ArgumentParser(description="Gate init zero ablation training for ChemGraphormer with warmup + cosine LR decay.")
...
def run_gate_init_zero_ablation(args):
    """
    Full gate init zero ablation training loop for ChemGraphormer with linear warmup + cosine LR decay,
    ...
    no_improve_epochs = 0
    best_ckpt_path = os.path.join(args.ckpt_dir, "chemgraphormer_gate_zero_ablation_best_model.pt")
